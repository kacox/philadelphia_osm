...
Code to clean OSM XML data, load into a Python dictionary, and write 
as a JSON file.
"""
...
def load_by_line(file_in):
    ...
    Iterparses through the file, processing each node then writing 
    that node into a JSON file.
    """
...
def process_node(element):
    ...
    Stores specified, cleaned information into a
    dictionary entry.
        ...
        if tag.attrib['k'].split(":")[0] == "addr":
            ...
            if temp_key in ADDRESS:
                if temp_key == "housenumber":
                    addr[temp_key] = check_val(tag.attrib['v'], '\d+')
    ...
    node["created"] = crtd
    if len(pos) > 0:
        lat_status = is_float(pos["lat"])
        lon_status = is_float(pos["lon"])
        if (lat_status == True) and (lon_status == True):
...
def is_float(val):
    ...
    Checks if the variable is a a float. Returns
...
def check_val(val, regex):
    ...
    Checks if the provided value matches the provided
    regular expression.

    Returns the value if a match exists; else
    'Invalid' is returned.
    """
...
def clean_street(street):
    ...
    Checks if the provided street name has any
    abbreviations.

    Returns the cleaned street name; if no cleaning
    is needed, the street name is returned as
    provided.
    """
